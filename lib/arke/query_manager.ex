...
defmodule Arke.QueryManager do
  ...
  alias Arke.Core.{Arke, Unit, Query, Parameter, Link}
  ...
  def create(project, arke, args) do
    ...
    with %Unit{} = unit <- Unit.load(arke, args, :create),
         ...
         {:ok, unit} <- run_group_and_link_hooks(arke, unit, :create, :after) do
      {:ok, unit}
    else
      {:error, errors} -> {:error, errors}
      %{errors: [{_, err} | _], valid: _} -> {:error, err}
    end
  end
  ...
  # todo: enhance for different arke types that acts as tables
  defp prepare_update_bulk_units(%{id: :arke_link} = arke, unit_list, data) do
    data_map =
      data
      |> Enum.map(fn d ->
        key = {d.parent_id, d.child_id, d.type}
        {key, d}
      end)
      |> Map.new()
  ...
  defp build_link_entries(%{data: %{multiple: true}} = parameter, unit, old_value, new_value) do
    Enum.reduce(old_value, {[], []}, fn value, {add_acc, del_acc} ->
      {[update_link_entry(parameter, unit, value) | add_acc],
       [update_link_entry(parameter, unit, value) | del_acc]}
    end)
  end
  ...
  defp build_link_entries(parameter, unit, old_value, new_value) when is_nil(new_value) do
    {[], [update_link_entry(parameter, unit, old_value)]}
  end

  defp build_link_entries(parameter, unit, old_value, new_value) when is_nil(old_value) do
    {[update_link_entry(parameter, unit, new_value)], []}
  end

  defp build_link_entries(parameter, unit, old_value, new_value) do
    {[update_link_entry(parameter, unit, new_value)],
     [update_link_entry(parameter, unit, old_value)]}
  end
  ...
  defp process_bulk(valid, errors, arke, persistence_fn, phase) do
    with {:ok, valid, errors} <-
           run_bulk_persistence_hook(valid, errors, arke, persistence_fn, phase),
         {:ok, valid, errors} <-
  ...
  defp run_bulk_persistence_hook(valid, errors, arke, :create, :before),
    do: ArkeManager.call_func(arke, :before_bulk_create, [arke, valid, errors])

  defp run_bulk_persistence_hook(valid, errors, arke, :create, :after),
    do: ArkeManager.call_func(arke, :on_bulk_create, [arke, valid, errors])

  defp run_bulk_persistence_hook(valid, errors, arke, :update, :before),
    do: ArkeManager.call_func(arke, :before_bulk_update, [arke, valid, errors])

  defp run_bulk_persistence_hook(valid, errors, arke, :update, :after, current_units),
    do: ArkeManager.call_func(arke, :on_bulk_update, [arke, current_units, valid, errors])

  defp run_bulk_persistence_hook(valid, errors, arke, :delete, :before),
    do: ArkeManager.call_func(arke, :before_bulk_delete, [arke, valid, errors])

  defp run_bulk_persistence_hook(valid, errors, arke, :delete, :after),
